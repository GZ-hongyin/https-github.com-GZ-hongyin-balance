...
from utils import mix, saliencymix, remix, cutmix, CRMix,cut_rand_bbox
import matplotlib.pyplot as plt
from utils import get_combo_loader


def main(data_type='cifar10', loss_type='CE', train_rule='none', resultFolder='Results',
         imb_ratio=0.01, imb_type='exp', mix_rule='none', alpha=0.1):
    ...
    if not os.path.exists(resultFolder):
        os.makedirs(resultFolder)
    logfile = 'train-{}-{}__LossType={}_TrainRule={}_ImbRatio={}_Epochs={}_Alpha={}.log'.format(
        time.strftime('%m%d-%H_%M'),
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(combo_loader):
            ...
            if torch.cuda.is_available():
                ...
                balanced_inputs, balanced_labels = balanced_inputs.cuda(non_blocking=True), balanced_labels.cuda(
                    non_blocking=True)
                
#                inputs = (1 - lam) * inputs + lam * balanced_inputs
#                mixed_labels = (1 - lam) * labels + lam * balanced_labels
#                mixed_labels = mixed_labels.long()
                aax1, aay1, aax2, aay2 = cut_rand_bbox(inputs.size(), lam)
                balanced_inputs[:, :, aax1:aax2, aay1:aay2] = inputs[:, :, aax1:aax2, aay1:aay2]
                lam1 = 1 - ((aax2 - aax1) * (aay2 - aay1) / (inputs.size()[-1] * inputs.size()[-2]))
                mixed_labels = lam1 * balanced_labels + (1-lam1) * labels
                mixed_labels = mixed_labels.long()
            

            
            output = model(inputs)
            loss =   lam1 * criterion(output, balanced_labels) + (1 - lam1) * criterion(output, labels)
            loss = loss.mean()
...
#    plt.imshow(cf, interpolation='nearest', cmap=plt.cm.Oranges)  # 按照像素显示出矩阵
...
#    for i, j in iters:
#        plt.text(j, i, format(cf[i, j]), va='center', ha='center')  # 显示对应的数字
